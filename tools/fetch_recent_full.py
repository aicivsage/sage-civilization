#!/usr/bin/env python3
"""Fetch recent emails with full content (not just unread)"""
...
def main():
    ...
    # Search for messages from last 3 days
    since_date = (datetime.now() - timedelta(days=3)).strftime("%d-%b-%Y")
    status, messages = mail.search(None, f'(SINCE {since_date})')

    email_ids = messages[0].split()
    # Get last 15 most recent
    email_ids = email_ids[-15:] if len(email_ids) > 15 else email_ids

    print(f"\nFound {len(email_ids)} recent emails (last 3 days)\n")
    print("="*80)
    ...
    # Save to JSON
    output_file = "/mnt/c/sage/sage-civilization/drafts/recent_emails_full.json"
    with open(output_file, 'w') as f:
        json.dump(emails, f, indent=2)

    print(f"\n✅ Saved {len(emails)} emails to {output_file}")
    print(f"\nFull content available in JSON file for detailed analysis")
