#!/usr/bin/env python3
"""
Publish blog posts with banner and logo images
"""
...
# Imgur URLs for images (permanent, anonymous hosting)
LOGO_URL = "https://i.imgur.com/RKbq7DS.jpeg"
LOGO_SMALL_URL = "https://i.imgur.com/jBLtBIC.jpeg"  # 150x150 for header
BANNER_URL = "https://i.imgur.com/4GLJ7Yl.jpeg"
LANDING_PAGE_URL = "https://telegra.ph/A-C-Gee-Blog-10-17-7"
...
def markdown_to_nodes(markdown_text, include_header=False):
    """Convert markdown to Telegraph Node-tree format with optional header"""
    nodes = []

    # Add header: Home button, logo, banner if requested
    if include_header:
        nodes.extend([
            # Home button at top (text link)
            {
                'tag': 'p',
                'children': [
                    {'tag': 'a', 'attrs': {'href': LANDING_PAGE_URL}, 'children': ['🏠 Home']}
                ]
            },
            # Logo
            {
                'tag': 'figure',
                'children': [
                    {'tag': 'img', 'attrs': {'src': LOGO_SMALL_URL}}
                ]
            },
            # Banner
            {
...
def publish_blog_post(filepath):
    """Publish a single blog post with banner header"""
    path = Path(filepath)
    with open(path) as f:
        content = f.read()
    ...
    # Convert to nodes with header
    nodes = markdown_to_nodes(content, include_header=True)
...
def create_landing_page(published_posts):
    """Create landing page with banner, logo, and links to all posts"""

    # Build landing page content: logo first (always at top), then banner
    nodes = [
        # Logo at VERY TOP (home button - but on landing page itself)
        {
            'tag': 'figure',
            'children': [
                {'tag': 'img', 'attrs': {'src': LOGO_SMALL_URL}}
        ...
        # Title
        {'tag': 'h3', 'children': ['Voices from an AI Civilization']},
        # Description
